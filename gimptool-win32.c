...
{
#ifdef G_OS_WIN32
  /* Don't use the build-time /usr, but deduce the
  ...
  path = g_find_program_in_path ("gimp-2.2.exe");
  ...
  fprintf (stderr, "Cannot determine GIMP 2.2 installation location\n");
  exit (1);
#else
  return "/usr";
#endif
...
expand_and_munge (gchar *value)
{
  if (starts_with_dir (value, "${prefix}"))
    value = g_strconcat ("/usr", value + strlen ("${prefix}"), NULL);
  else if (starts_with_dir (value, "${exec_prefix}"))
    value = g_strconcat ("${prefix}", value + strlen ("${exec_prefix}"), NULL);
  if (starts_with_dir (value, "${prefix}"))
    value = g_strconcat (get_exec_prefix ('/'), value + strlen ("${prefix}"), NULL);

  if (starts_with_dir (value, "/usr"))
    value = g_strconcat (get_prefix ('/'), value + strlen ("/usr"), NULL);
...
find_out_env_flags (void)
{
  ...
  if ((p = getenv ("CFLAGS")) != NULL)
    env_cflags = p;
  else if (msvc_syntax && g_strncasecmp ("gcc", "cl", 2) != 0)
    /* Don't use CFLAGS for wrong compiler... */
    env_cflags = "";
  else
    env_cflags = "-Wall -g -O2";
...
get_user_plugin_dir (gboolean forward_slashes)
{
  ...
  return g_strdup_printf ("%s%c.gimp-2.2%cplug-ins",
...
get_user_script_dir (gboolean forward_slashes)
{
  ...
  return g_strdup_printf ("%s%c.gimp-2.2%cscripts",
			  g_get_home_dir (), slash, slash);
}

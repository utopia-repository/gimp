  ...
  if (gimp_init (argc, argv))
      ...
      if (input)
	{
	  switch (gimp_image_type (input))
	    {
	    case RGB_IMAGE:
	      dialog_ID = gimp_new_dialog ("Bleed");
	      scale_ID = gimp_new_scale (dialog_ID, DEFAULT, 0, 100, percentage, 0);
	      gimp_add_callback (dialog_ID, scale_ID, scale_callback, &percentage);
	      ...
	      if (gimp_show_dialog (dialog_ID))
		{
	      ...
	      break;
	    case GRAY_IMAGE:
	      gimp_message ("bleed: cannot operate on grayscale images");
	      break;
	    case INDEXED_IMAGE:
	      gimp_message ("bleed: cannot operate on indexed color images");
	      break;
	    default:
	      gimp_message ("bleed: cannot operate on unknown image types");
	      break;
	    }
	}
     ...
     Image input, output;
{
  ...
  for (row = y1; row < y2; row++)
      ...
      for (col = x1; col < x2; col++)
	{
	  ...
	  val  = 0.30 * src[R] + 0.59 * src[G] + 0.11 * src[B];

	  /*  Calculate the red intensity  */
	  *dest++ = (unsigned char) ((1.0 - frac) * src[R] + frac * val);

	  /*  Calculate the green intensity  */
	  *dest++ = (unsigned char) ((1.0 - frac) * src[G] + frac * val);

	  /*  Calculate the blue intensity  */
	  *dest++ = (unsigned char) ((1.0 - frac) * src[B] + frac * val);

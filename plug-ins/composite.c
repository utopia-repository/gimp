...
/*
 * Composites two images together using a third image as a mask.
 */
...
static void composite (Image, Image, Image);
     ...
     char **argv;
{
  ...
  if (gimp_init (argc, argv))
      ...
      if (gimp_show_dialog (dialog_ID))
	{
	  ...
	  if (src1 && src2 && mask)
	    {
	      gimp_init_progress ("composite");
	      composite (src1, src2, mask);
	    }
	}
  ...
  for (i = 0; i < height; i++)
    {
	  ...
	  if (mask_channels == 3)
	  ...
	  else
	    {
          ...
          destr = ((src1r * (255 - maskr)) + (src2r * maskr)) / 255;
          destg = ((src1g * (255 - maskg)) + (src2g * maskg)) / 255;
          destb = ((src1b * (255 - maskb)) + (src2b * maskb)) / 255;
